...
export default function TeamsPage({ onBack }: TeamsPageProps): JSX.Element {
      ...
      <div className="flex-1 flex flex-col items-center pt-8 px-4 w-full max-w-xl mx-auto">
        <div className="w-full bg-[#fdf2d1] border-2 border-black p-6 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-between items-center bg-[#60a5fa] border-2 border-black p-3">
              <span className="font-bold">Team Colors</span>
              <div className="w-12 h-6 bg-blue-500 border border-black"></div>
            </div>
            <div className="flex justify-between items-center bg-[#60a5fa] border-2 border-black p-3">
              <span className="font-bold">Private Lobby</span>
              <div className="w-12 h-6 bg-gray-300 rounded-full border border-black relative">
                <div className="absolute left-1 top-1 w-4 h-4 bg-white rounded-full border border-black"></div>
              </div>
            </div>
          </div>
          <div className="flex gap-4 mt-8">
            <button onClick={() => setView("choice")} className="flex-1 py-2 bg-white border-2 border-black font-bold rounded-full">CANCEL</button>
            <button onClick={() => setView("lobby")} className="flex-1 py-2 bg-[#f7e49a] border-2 border-black font-bold rounded-full shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">CREATE TEAM</button>
          </div>
        </div>
      </div>
    );
    ...
    return (
        <div className="flex-1 flex flex-col items-center pt-8 px-4 w-full max-w-2xl mx-auto relative">
             <div className="absolute right-0 top-0 opacity-20 pointer-events-none">
                ...
                <div className="w-64 h-64 bg-black rounded-full blur-3xl"></div>
             </div>
             <div className="w-full">
                <h2 className="text-2xl font-bold mb-4 italic underline underline-offset-4">OPEN TEAMS</h2>
                <div className="space-y-4">
                    {["TEAM A", "Tigers"].map((name) => (
                        <div key={name} className="flex items-center justify-between bg-orange-300 border-2 border-black p-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                            <div>
                                <p className="font-bold text-lg">{name}</p>
                                <p className="text-xs font-bold">1/8 Players</p>
                            </div>
                            <button onClick={() => setView("lobby")} className="bg-white border-2 border-black px-8 py-1 font-bold hover:bg-gray-100 uppercase">Join</button>
                        </div>
                    ))}
                </div>
  ...
  // --- SUB-VIEW: 4. LOBBY (Waiting for Players) ---
  return (
    <div className="flex-1 flex flex-col items-center pt-8 px-4 w-full max-w-5xl mx-auto">
      <div className="w-full bg-[#60a5fa] border-4 border-black p-6 shadow-[10px_10px_0px_0px_rgba(0,0,0,1)] relative">
        <h2 className="text-3xl font-black mb-12 uppercase italic">Team B</h2>
        <div className="grid grid-cols-3 gap-y-12 mb-20">
          {[1, 2, 3].map((num) => (
            <div key={num} className="flex flex-col items-center">
              <div className="w-24 h-24 rounded-full border-4 border-black bg-gray-200 mb-2 overflow-hidden shadow-md">
                <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${num}`} alt="player" />
              </div>
              <span className="bg-white border-2 border-black px-4 py-0.5 text-xs font-bold">Player {num}</span>
            </div>
          ))}
        ...
        <div className="absolute bottom-[-24px] left-1/2 -translate-x-1/2">
             <button className="bg-[#f7e49a] border-2 border-black px-12 py-3 font-bold text-xl shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] hover:translate-y-1 hover:shadow-none transition-all uppercase">
                Join Team
             </button>
        </div>
      </div>
