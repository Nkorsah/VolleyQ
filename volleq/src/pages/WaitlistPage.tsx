...
export default function WaitlistPage({ onBack }: Props): JSX.Element {
  ...
  const [winningScore, setWinningScore] = useState(25); // Default to 25, toggleable to 18
  const [winner, setWinner] = useState<string | null>(null);

  // Mock data for Court Selection
  const courts = [
  ...
  // --- 2. SCOREBOARD VIEW (iPad Optimized) ---
  if (view === "scoreboard") {
    return (
      <div className="flex-1 flex flex-col items-center justify-center px-4 w-full h-full min-h-screen">
        <h2 className="text-3xl font-black mb-4 italic uppercase">Current Match - {selectedCourt}</h2>
        ...
        <div className="flex w-full max-w-6xl aspect-[16/10] border-4 border-black rounded-[40px] overflow-hidden shadow-[16px_16px_0px_0px_rgba(0,0,0,1)] relative bg-black">
          ...
          <div className="flex-1 bg-orange-500 flex flex-col items-center justify-center border-r-4 border-black relative">
             ...
             <div className="absolute inset-y-0 left-6 flex flex-col justify-center gap-6">
                <button 
                  ...
                  className="w-20 h-20 bg-white border-4 border-black rounded-2xl flex items-center justify-center text-5xl font-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:translate-y-1 active:shadow-none transition-all"
                >+</button>
                <button 
                  onClick={() => updateScore('A', -1)}
                  className="w-20 h-20 bg-orange-200 border-4 border-black rounded-2xl flex items-center justify-center text-5xl font-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:translate-y-1 active:shadow-none transition-all"
                >-</button>
             </div>
          </div>
          ...
          <div className="flex-1 bg-blue-500 flex flex-col items-center justify-center relative">
             ...
             <div className="absolute inset-y-0 right-6 flex flex-col justify-center gap-6">
                ...
                <button 
                  onClick={() => updateScore('B', -1)}
                  className="w-20 h-20 bg-blue-200 border-4 border-black rounded-2xl flex items-center justify-center text-5xl font-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:translate-y-1 active:shadow-none transition-all"
                >-</button>
             </div>
          </div>
          ...
          {winner && (
            <div className="absolute inset-0 bg-black/95 backdrop-blur-xl flex flex-col items-center justify-center z-50 animate-in fade-in zoom-in duration-300">
               <h3 className="text-white text-7xl font-black italic mb-2 tracking-tighter">GAME OVER!</h3>
               <p className="text-yellow-400 text-4xl font-black uppercase mb-8">{winner} is the winner!</p>

               <div className="flex items-center gap-10 mb-12 bg-white/10 p-10 rounded-[32px] border-2 border-white/20">
                  <div className="text-center">
                     <p className="text-orange-400 font-black text-sm uppercase mb-2">Team A</p>
                     <p className="text-white text-8xl font-black">{scoreA}</p>
                  </div>
                  <div className="text-white text-5xl font-thin opacity-30 italic">vs</div>
                  <div className="text-center">
                     <p className="text-blue-400 font-black text-sm uppercase mb-2">Team B</p>
                     <p className="text-white text-8xl font-black">{scoreB}</p>
                  </div>
               </div>

               <button 
                onClick={resetGame}
                className="bg-yellow-400 border-4 border-black px-16 py-6 font-black text-3xl shadow-[8px_8px_0px_0px_rgba(255,255,255,0.4)] active:scale-95 transition-all uppercase"
               >
                 Start New Match
               </button>
            </div>
        ...
        <button 
          ...
          className="mt-10 flex flex-col items-center group"
        >
          <span className="font-bold text-sm underline group-hover:no-underline transition-all uppercase tracking-widest">Click to see queue</span>
  ...
  // --- 3. QUEUE DETAIL VIEW ---
  return (
    <div className="flex-1 flex flex-col items-center pt-8 px-4 w-full min-h-screen">
      <button onClick={() => setView("scoreboard")} className="text-5xl font-black mb-4 hover:scale-110 transition-transform">⌃</button>
      ...
      <div className="w-full max-w-xl flex flex-col gap-6">
        <div className="bg-gradient-to-r from-orange-400 to-orange-600 border-4 border-black p-6 text-center shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
           <span className="text-2xl font-black uppercase text-white tracking-widest">Current: Team A vs Team B</span>
        </div>

        <h3 className="text-2xl font-black italic text-center mt-6 uppercase">Next Up In Queue</h3>
      ...
      <button 
        ...
        className="mt-16 font-black uppercase border-b-4 border-black text-xl hover:bg-black hover:text-white px-4 transition-all"
      >
        Back to Courts
      </button>
    </div>
