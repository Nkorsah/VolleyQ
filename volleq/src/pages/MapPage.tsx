        ...
        <main className="flex-1 flex flex-col relative bg-cover bg-center overflow-y-auto"
          ...
          {activeSubView === "menu" && (
            <div className="flex flex-col items-center pt-12">
              <button onClick={() => setShowFullDetails(false)} className="absolute top-4 left-4 text-sm font-bold text-gray-600 hover:underline">← Back to Map</button>
              ...
              <div className="flex flex-col gap-6 w-full max-w-md px-6">
                <button onClick={() => setActiveSubView("teams")} className="w-full py-6 bg-[#f7e49a] border border-gray-400 rounded-xl text-xl font-medium shadow-sm hover:bg-[#f2db82]">Teams</button>
                <button onClick={() => setActiveSubView("waitlist")} className="w-full py-6 bg-[#f7e49a] border border-gray-400 rounded-xl text-xl font-medium shadow-sm hover:bg-[#f2db82]">Waitlist Queue</button>
                <button onClick={() => setActiveSubView("host")} className="w-full py-6 bg-[#f7e49a] border border-gray-400 rounded-xl text-xl font-medium shadow-sm hover:bg-[#f2db82]">Host Court</button>
              </div>
            </div>
          )}

          {activeSubView === "teams" && <TeamsPage onBack={() => setActiveSubView("menu")} />}
          {activeSubView === "waitlist" && <WaitlistPage onBack={() => setActiveSubView("menu")} />}
